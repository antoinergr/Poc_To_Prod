...
class BaseTextCategorizationDataset:
    ...
    def _get_num_test_samples(self):
        ...
        (test set size)
        """
        return integer_floor(self._get_num_samples() * (Decimal(1)-Decimal(self.train_ratio)))
    ...
    def to_indexes(self, labels):
        ...
        """
        return [self.get_label_to_index_map()[x] for x in labels]
...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        assert data_set.columns.to_list() == ['post_id', 'tag_name', 'tag_id', 'tag_position', 'title']
        ...
        # TODO: CODE HERE
        return (data_set.pipe(filter_tag_position(0)).pipe(filter_tags_with_less_than_x_samples(min_samples_per_label)))
